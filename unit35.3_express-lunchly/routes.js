...
router.post("/search", async function(req, res, next) {
  try {
    ...
    const formatted_searchName = searchName[0].toUpperCase() + searchName.slice(1);
    const customers = await Customer.searchBy(formatted_searchName);
...
router.post("/:id/edit-reservation/:rid", async function(req, res, next) {
  try {
    const reservation = await Reservation.get(req.params.rid);
...
router.post("/:id/delete-reservation/:rid", async function(req, res, next) {
  try {
    ...
    const results = await Reservation.delete(req.params.rid);
