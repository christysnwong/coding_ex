...
def show_likes(user_id):
    ...
    user = User.query.get_or_404(user_id)
    like_messages = user.likes

    return render_template('users/likes.html', user=user, messages=like_messages)


@app.route('/messages/<int:message_id>/like', methods=['POST'])
def toggle_like(message_id):
    """Add/remove a like for the currently-logged-in user."""
    ...
    if message.user_id == g.user.id:
        # return abort(403)
        flash('Sorry you cannot add your own message to your favourite!', 'info')
    elif message not in user_likes:
        g.user.likes.append(message)
        flash('This message is added to your favourite!', 'success')
    else:
        g.user.likes = [like for like in user_likes if like != message]
        flash('This message is removed from your favourite!', 'success')
...
def profile():
    ...
    if user:
        ...
        if form.validate_on_submit():
            ...
            correct_user = User.authenticate(username, password)
